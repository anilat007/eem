...
import com.kbit.eem.service.Configurations.StudentAdmission.ReligionService;
import com.kbit.eem.utils.Helper;
import com.kbit.eem.utils.ServiceUIMapper;

@Controller
@RequestMapping(ApiPaths.Config.Student.religion)
public class ReligionApi extends JSonRepositoryBaseController<Religion, ReligionSO> {
	static Logger logger = Logger.getLogger(ReligionApi.class);	
	Repository<ReligionSO> repository = new ReligionService();
	IServiceUiMapper<Religion, ReligionSO> mapper = new ServiceUIMapper<Religion, ReligionSO>(
			Religion.class, ReligionSO.class);
	...
	public @ResponseBody
	JSonResponse addItem(@RequestBody Religion itemToAdd) {
		try {
			ReligionSO item = mapper.MapToServiceModel(itemToAdd);
	...
	public @ResponseBody
	JSonResponse udpateItem(@RequestBody Religion itemToUpdate) {
		try {
			ReligionSO item = mapper.MapToServiceModel(itemToUpdate);
			item.setUserId(this.getUserName());
