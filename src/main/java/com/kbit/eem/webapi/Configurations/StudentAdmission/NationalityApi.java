...
import com.kbit.eem.models.ui.Configurations.StudentAdmission.Nationality;
...
import com.kbit.eem.service.Configurations.StudentAdmission.NationalityService;
import com.kbit.eem.utils.Helper;
import com.kbit.eem.utils.ServiceUIMapper;


@Controller
@RequestMapping(ApiPaths.Config.Student.nationality)
public class NationalityApi extends JSonRepositoryBaseController<Nationality, NationalitySO> {
	static Logger logger = Logger.getLogger(NationalityApi.class);	
	Repository<NationalitySO> repository = new NationalityService();
	IServiceUiMapper<Nationality, NationalitySO> mapper = new ServiceUIMapper<Nationality, NationalitySO>(
			Nationality.class, NationalitySO.class);
	...
	public @ResponseBody
	JSonResponse addItem(@RequestBody Nationality itemToAdd) {
		try {
			NationalitySO item = mapper.MapToServiceModel(itemToAdd);
	...
	public @ResponseBody
	JSonResponse udpateItem(@RequestBody Nationality itemToUpdate) {
		try {
			NationalitySO item = mapper.MapToServiceModel(itemToUpdate);
			item.setUserId(this.getUserName());
